...
- [ ] **4.1** 新增 `image_inspect`。（现在实现）
- [ ] **4.2** 新增 `image_tag`。（后续实现）
- [ ] **4.3** 新增 `image_prune`。（后续实现）
- [ ] **4.4** 新增 `image_build`，并支持有界的构建进度处理。（后续实现）
- [ ] **4.5** 新增 `image_push`，并支持有界的 push 进度处理。（后续实现）
- [ ] **4.6** 新增 `image_load` 和 `image_save`，并处理二进制/tar 流。（后续实现）
