...
internal sealed class McpToolRegistry : IMcpToolRegistry
{
    ...
    private Dictionary<string, ToolEntry> BuildTools()
    {
        ...
        var tools = new[]
        {
            ...
            T("container_logs", "Read container logs.", new JsonObject { ["idOrName"] = StringSchema("Container ID or name."), ["tail"] = StringSchema("Number of lines or all."), ["since"] = StringSchema("Timestamp or duration accepted by the runtime."), ["timestamps"] = BoolSchema() }, ["idOrName"], _containers.ContainerLogsAsync),
