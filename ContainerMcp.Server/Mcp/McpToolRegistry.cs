...
internal sealed class McpToolRegistry : IMcpToolRegistry
{
    ...
    private Dictionary<string, ToolEntry> BuildTools()
    {
        ...
        var tools = new[]
        {
            ...
            T("container_prune", "Prune stopped containers.", new JsonObject { ["until"] = StringSchema("Prune containers stopped before this timestamp or duration."), ["labels"] = StringArraySchema("Label filters."), ["labelNe"] = StringArraySchema("Negative label filters.") }, null, _containers.ContainerPruneAsync),
            ...
            T("container_logs_follow", "Follow container logs for a bounded duration.", new JsonObject { ["idOrName"] = StringSchema("Container ID or name."), ["durationSeconds"] = IntSchema(), ["tail"] = StringSchema("Number of lines or all."), ["timestamps"] = BoolSchema(), ["maxBytes"] = IntSchema() }, ["idOrName"], _containers.ContainerLogsFollowAsync),
            T("volume_list", "List container volumes.", new JsonObject(), null, _volumes.VolumeListAsync),
            T("volume_inspect", "Inspect a container volume.", new JsonObject { ["name"] = StringSchema("Volume name.") }, ["name"], _volumes.VolumeInspectAsync),
